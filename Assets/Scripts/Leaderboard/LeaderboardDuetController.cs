...
public class DuetScoreComparer:IComparer<UserAccount>
{

    public DuetScoreComparer() { }
    public int Compare(UserAccount u1, UserAccount u2)
    {
        return u1.highestScoreDuet.CompareTo(u2.highestScoreDuet);
    }
}


public class LeaderboardDuetController : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        try
        {
            ...
            if (allUsers != null)
            {
                ...
                {
                    if (allUsers[i].highestScoreDuet != 0)
                    {
                ...
                {
                    peopleOnBoard.Sort(new DuetScoreComparer());
                    ...
                    if (peopleOnBoard.Count < 5)
                        for (int i = 0; i < peopleOnBoard.Count; i++)
                        {
                            ...
                            top5_scores[i].text = peopleOnBoard[i].highestScoreDuet.ToString();
